...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
    ...
    if (less(node->datum, query)) return find_impl(node->right, query, less);
    return node;
  }
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    if (node == nullptr) return new Node(item, nullptr, nullptr);
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    if (node->left != nullptr) {
      Node *max_left_node = max_element_impl(node->left);
      if (!less(max_left_node->datum, node->datum)) {
        return false;
      }
    }

    if (node->right != nullptr) {
      Node *min_right_node = min_element_impl(node->right);
      if (!less(node->datum, min_right_node->datum)) {
        return false;
      }
    }
